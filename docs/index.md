...
# DA-1 of individual team members
...
## Documentation for contributions by Sreenath S (20BCE1450) - DA1
...
## Documentation for contributions by Hemang Ahuja (20BCE1302) - DA1

## Load Balancer Documentation

### Introduction

A load balancer is an essential component of any web infrastructure that handles a high volume of traffic. It distributes incoming traffic across multiple servers to ensure that no server is overwhelmed with requests, leading to better performance, scalability, and availability. In this project, we will implement a load balancer using TypeScript that proxies requests to multiple servers using different load balancing algorithms, such as round-robin, weighted round-robin, and URL hash.

### Architecture

The load balancer we will implement consists of two main components: a server component and a proxy component. The server component is responsible for receiving incoming requests from clients and forwarding them to the appropriate backend server based on the selected algorithm. The proxy component is responsible for forwarding the requests to the backend servers and returning the responses to the client.

### Load Balancing Algorithms

We will implement three load balancing algorithms in our load balancer.

1. Round-Robin

The round-robin algorithm distributes requests equally among theavailable servers in a circular order. For example, if there are three backend servers, the first request goes to server 1, the second request goes to server 2, the third request goes to server 3, and so on. After the last server receives a request, the algorithm starts over from the first server.

2. Weighted Round-Robin: The weighted round-robin algorithm assigns a weight to each server based on its processing power. The load balancer distributes requests proportionally to each server's weight. For example, if server 1 has a weight of 2, and server 2 has a weight of 1, the first two requests go to server 1, and the third request goes to server 2. After the last server receives a request, the algorithm starts over from the first server.

3. URL Hash: The URL hash algorithm assigns a request to a server based on a hash of the request's URL. The algorithm ensures that all requests with the same URL go to the same server. This is useful when we have stateful applications that require all requests to be handled by the same server.

### Load Balancer Implementation:

We will implement the load balancer using TypeScript, which is a superset of JavaScript that adds type annotations and other features to JavaScript. The load balancer will be implemented using the following steps:

-   Start by defining the server component, which listens for incoming requests
-   from clients. We can use the Node.js http module to create a server.
-   Define the backend servers as an array of objects, where each object represents a server and contains its address, port, and weight.

# DA-2 of individual team members
...
## Commits by Hemang Ahuja (20BCE1302)

1. **[initial commit · hemangahuja/pyjamas_forms@111b62f · GitHub](https://github.com/hemangahuja/pyjamas_forms/commit/111b62f35093dfef5b0b8cf9b9b2f4672839f64e)**

-   This commit is the first commit of the project.
-   Commit hash: 111b62f35093dfef5b0b8cf9b9b2f4672839f64e

2. **[add lookup and primiary key feature · hemangahuja/pyjamas_forms@cda0093 · GitHub](https://github.com/hemangahuja/pyjamas_forms/commit/cda0093e23913f839f1cf98755a7e7ce217f4797)**
...
3. **[implement encryption · hemangahuja/pyjamas_forms@dc25cc9 · GitHub](https://github.com/hemangahuja/pyjamas_forms/commit/dc25cc96358aa3fbb22adc53b4bbb10e5bd4ae69)**
...
4. **[added load_balancer and logging · hemangahuja/pyjamas_forms@926ff19 · GitHub](https://github.com/hemangahuja/pyjamas_forms/commit/926ff191337982c64389f04b8e7c471b04aa0f93)**
...
4. **[add comments to contributions · hemangahuja/pyjamas_forms@1bc7ca4 · GitHub](https://github.com/hemangahuja/pyjamas_forms/commit/1bc7ca42ae1e5d17547a94e38060bbfa3a58f44e)**

-   This commit adds comments to contributions

-   Commit hash: 1bc7ca42ae1e5d17547a94e38060bbfa3a58f44e

## Commits by Sreenath S (20BCE1450)
...
# DA-3 of individual team members

## Comments in code by Vishal N (20BCE1043)
...
## Comments in code by Hemang Ahuja (20BCE1302)

1. load_balancer.ts

-   1. configures the load balancer to point to the specified port using the specified algorithm
...
-   2. creates proxy instance for request forwarding

    ```typescript
    const proxy = httpProxy.createProxyServer({});
    ```

-   3. hashes a string to a number

    ```typescript
    const hashCode = (str: string): number
    ```

-   4. returns index of the server to be used using different algorithms for load balancing.

    ```typescript
    const getServer = (options?: object): number
    ```

-   5. processes the request and returns the response after balancing the load

    ```typescript
    const handler = async (req: express.Request, res: express.Response)
    ```

-   6. calculate server address using algorithm and base port
...
-   7. sends request to the server

    ```typescript
    proxy.web(req, res, { target: server });
    ```

-   8. starts the server at the specified port
...
## Comments in code by Sreenath S (20BCE1450)
